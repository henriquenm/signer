...
class Signer
  ...
  def security_node
    @security_node.xpath("/*[name()='NFe']/*[name()='infNFe']").first# ||= document.xpath('//wsse:Security', wsse: WSSE_NAMESPACE).first
  end
  ...
  def digest!(target_node, options = {})
    ...
    target_node = target_node.to_xml(:save_with => Nokogiri::XML::Node::SaveOptions::AS_XML)
